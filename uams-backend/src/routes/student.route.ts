...
import {
  ...
  getPhotoUploadSignedUrl, revealStudentNid, getStudentCourses,
...
studentRoute.get('/me/courses', rbac('student'), async (c) => {
  const { tenantId, sub } = c.get('user')
  try {
    return c.json({ data: await getStudentCourses(tenantId, sub) })
  } catch (err: any) {
